# Welcome to tomial-clicky-tooth's test suite!
...
tomial-clicky-tooth aims to be out-the-box compatible with
...
that PyInstaller does need special help when compiling tomial-clicky-tooth
then that *help* goes in a *hook* file (see
`tomial_clicky_tooth/__pyinstaller/hook-tomial_clicky_tooth.py`).
...
changed the tomial_clicky_tooth source code.
...
This executable is just `pytest.main()` with tomial_clicky_tooth
...
Otherwise you may unintentionally use the original source tomial_clicky_tooth
...
If you've modified either the source code or the hooks for tomial_clicky_tooth
...
`tomial_clicky_tooth/__pyinstaller/`).
